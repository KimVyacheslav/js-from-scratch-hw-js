...
const webTech = document.getElementById("web-tech-image")
let numberList = 0;
nextBtn.addEventListener("click", function () {
	if (numberList < WEB_TECH_IMAGES.length - 1) {
		numberList += 1
	} else {
		numberList = 0
	}
	webTech.src = WEB_TECH_IMAGES[numberList]
})

prevBtn.addEventListener("click", function () {
	if (numberList === 0) {
		numberList = WEB_TECH_IMAGES.length - 1
	} else {
		numberList -= 1
	}
	webTech.src = WEB_TECH_IMAGES[numberList]
